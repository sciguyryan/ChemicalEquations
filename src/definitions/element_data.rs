...
lazy_static! {
    pub static ref ELEMENT_DATA: AllElementData = {
        AllElementData::new()
    };
}
...
impl AllElementData {
    ...
    fn initialize(&mut self) {
        ...
        let mut stmt = conn.prepare("SELECT atomic_number, symbol, name, atomic_weight FROM Elements").expect("Failed to prepare statement");

        let element_iter = stmt.query_map([], |row| {
            Ok(ElementData {
